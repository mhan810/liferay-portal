...
public class SystemEventLocalServiceImpl
	...
	public void addSystemEvent(
		...
		if (systemEventHierarchyEntry != null) {
			systemEvent.setEventSet(systemEventHierarchyEntry.getEventSet());
		}
		else {
			systemEvent.setEventSet(
				String.valueOf(counterLocalService.increment()));
		}

		if (systemEventHierarchyEntry.isAsset(classNameId, classPK)) {
			systemEvent.setEventId(systemEventHierarchyEntry.getEventId());
		}
		else {
			systemEvent.setEventId(
				SystemEventHierarchyEntryThreadLocal.getNextEventId(
					systemEvent.getEventSet()));
		}
