...
public class ExportPermissionsAdvice extends ExportImportAdvice
	...
	public void afterReturning(
		...
		if (exportPermissions) {
			Map<String, List<String>> permissionsList = null;
			...
			if (permissionsList != null) {
				...
				zipWriter.addEntry(
					ExportImportPathUtil.getPermissionPath(entity),
					xStream.toXML(permissionsList));
			}
		}
	}
	...
	private Map<String, List<String>> digestEntityPermissions(
		...
		Map<String, List<String>> roleNamesToActionIds =
		...
		for (Map.Entry<Long, String> entry : roleIdsToNames.entrySet()) {
			...
			roleNamesToActionIds.put(name, actionIdsList);
		}

		return roleNamesToActionIds;
	}

	protected Map<String, List<String>> digestPermissions(
			long companyId, long groupId, LayoutCache layoutCache,
		...
		for (Map.Entry<Long, Set<String>> entry :
			...
			permissions.put(String.valueOf(entry.getKey()), values);
		}
	...
	private Map<String, List<String>> digestLayoutPermissions(
		...
		return digestPermissions(
			companyId, groupId, layoutCache, resourceName, resourcePrimKey,
			false);
	}
