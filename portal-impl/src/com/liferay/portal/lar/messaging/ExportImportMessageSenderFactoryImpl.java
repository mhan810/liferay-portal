...
public class ExportImportMessageSenderFactoryImpl
	...
	public ExportImportMessageSender getExportImportMessageSender(
		...
		try {
			String dataHandlerClassName = dataHandlerClass.getName();

			ExportImportMessageSender exportImportMessageSender =
				_exportImportMessageSenders.get(dataHandlerClassName);

			if (exportImportMessageSender != null) {
				return exportImportMessageSender;
			}

			// Create new message sender

			exportImportMessageSender = new ExportImportMessageSenderImpl();
			...
			ExportImportMessageSender previousExportImportMessageSender =
				_exportImportMessageSenders.putIfAbsent(
					dataHandlerClassName, exportImportMessageSender);

			if (previousExportImportMessageSender != null) {
				exportImportMessageSender = previousExportImportMessageSender;
			}
	...
	private static final ConcurrentMap<String, ExportImportMessageSender>
		_exportImportMessageSenders =
			new ConcurrentHashMap<String, ExportImportMessageSender>();
