 ...
 */
public class LayoutsExportTask implements ExportImportTask {

	public LayoutsExportTask(BackgroundTask backgroundTask)
		...
		_groupId = backgroundTask.getGroupId();
		_privateLayout = data.getBoolean("privateLayout");

		JSONArray jsonArray = data.getJSONArray("layoutIds");

		_layoutIds = new long[jsonArray.length()];

		for (int i = 0; i < jsonArray.length(); i++) {
			_layoutIds[i] = jsonArray.getLong(i);
		}
		...
		jsonObject = jsonObject.getJSONObject("startDate");

		if (jsonObject != null) {
			_startDate = (Date)JSONFactoryUtil.deserialize(jsonObject);
		}

		jsonObject = jsonObject.getJSONObject("endDate");
	...
	public void run() throws PortalException, SystemException {
		...
		try {
			larFile = LayoutServiceUtil.exportLayoutsAsFile(
				_groupId, _privateLayout, _layoutIds, _parameterMap, _startDate,
				_endDate);
		}
		catch (Exception e) {
			data.putException(e);
			...
			_log.error("Unable to run layouts export task", e);
		}
