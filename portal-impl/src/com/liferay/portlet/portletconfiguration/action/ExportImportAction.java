...
public class ExportImportAction extends EditConfigurationAction {
	...
	protected void exportData(
		...
		try {
			...
			String taskData = ExportImportUtil.getExportPortletTaskData(
				plid, groupId, portlet.getPortletId(),
				actionRequest.getParameterMap(), startDate, endDate);

			BackgroundTask backgroundTask =
				BackgroundTaskLocalServiceUtil.addTask(
					themeDisplay.getUserId(), groupId,
					BackgroundTaskConstants.TASK_TYPE_EXPORT_PORTLET, fileName,
					taskData, new ServiceContext());

			MessageBusUtil.sendMessage(
				DestinationNames.EXPORT, backgroundTask.getBackgroundTaskId());
		}
		catch (Exception e) {
