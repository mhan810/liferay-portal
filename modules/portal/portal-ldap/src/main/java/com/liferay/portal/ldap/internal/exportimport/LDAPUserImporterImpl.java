...
public class LDAPUserImporterImpl implements LDAPUserImporter, UserImporter {
	...
	protected void addRole(
		...
		try {
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"Imported autogenerated role " + role + " from LDAP");
			}
		}
	...
	protected User addUser(long companyId, LDAPUser ldapUser, String password)
		...
		if (_log.isDebugEnabled()) {
			...
			_log.debug(
				"Adding ldapUser " + ldapUser + ", companyId = " + companyId);
		}
		...
		if (_log.isDebugEnabled()) {
			_log.debug(
				"Finished adding ldapUser " + ldapUser + " as user " +
	...
	protected Set<Long> importGroup(
		...
		if (_log.isDebugEnabled()) {
			_log.debug("Adding user " + user + " to group " + userGroupId);
		}
	...
	protected UserGroup importUserGroup(
		...
		try {
			...
			if (_log.isDebugEnabled()) {
				stopWatch.start();
				_log.debug("Adding ldapGroup " + ldapGroup + " to portal");
			}
			...
			try {
				...
				if (_log.isDebugEnabled()) {
					_log.debug(
						"Finished adding ldapGroup " + ldapGroup +
							" as userGroup " + userGroup +
							" to the portal in " + stopWatch.getTime() + "ms");
				}
			}
			catch (Exception e) {
				if (_log.isWarnEnabled()) {
	...
	protected void importUsers(
		...
		if (_log.isDebugEnabled()) {
			...
			_log.debug(
				"Importing " + attribute.size() + " user(s) from " +
		...
		for (int i = 0; i < attribute.size(); i++) {
			...
			try {
				...
				if (user != null) {
					if (_log.isDebugEnabled()) {
						_log.debug(
							"Adding user " + user + " to group " + userGroupId);
					}
		...
		if (_log.isDebugEnabled()) {
			_log.debug(
				"Finished importing " + newUserIds.size() + " of " +
					attribute.size() + " users from ldapServerId " +
		...
		for (User user : userGroupUsers) {
			if ((ldapServerId == user.getLdapServerId()) &&
				...
				if (_log.isDebugEnabled()) {
					_log.debug(
						"Removing user " + user + " from userGroup " +
							userGroupId);
				}
	...
	protected User updateUser(
		...
		if (_log.isDebugEnabled()) {
			...
			_log.debug(
				"Updating user " + user + " from ldapServerId " + ldapServerId +
					", companyId = " + companyId + ", isNew = " + isNew);
		}
