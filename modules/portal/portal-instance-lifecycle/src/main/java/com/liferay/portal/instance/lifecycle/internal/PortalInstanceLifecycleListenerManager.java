...
public class PortalInstanceLifecycleListenerManager {
	...
	protected void addCompany(Company company) {
		...
		for (PortalInstanceLifecycleListener portalInstanceLifecycleListener :
			...
			portalInstanceAdded(portalInstanceLifecycleListener, company);
		}
	}
	...
	protected void portalInstanceAdded(
		PortalInstanceLifecycleListener portalInstanceLifecycleListener,
		Company company) {
		...
		try {
			...
			portalInstanceLifecycleListener.portalInstanceAdded(company);
		}
		catch (PortalException pe) {
			if (_log.isWarnEnabled()) {
				_log.warn("Error registering portal instance: " + company, pe);
			}
		}
		finally {
	...
	protected void portalInstanceRemoved(
		PortalInstanceLifecycleListener portalInstanceLifecycleListener,
		Company company) {
		...
		try {
			...
			portalInstanceLifecycleListener.portalInstanceRemoved(company);
		}
		catch (PortalException pe) {
			if (_log.isWarnEnabled()) {
				_log.warn("Error registering portal instance: " + company, pe);
