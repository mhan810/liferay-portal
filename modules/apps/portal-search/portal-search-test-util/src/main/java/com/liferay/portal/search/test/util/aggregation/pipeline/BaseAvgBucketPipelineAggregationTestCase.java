...
public abstract class BaseAvgBucketPipelineAggregationTestCase
	...
	public void testAvgBucketPipeline() throws Exception {
		...
		AvgBucketPipelineAggregation avgBucketPipelineAggregation =
			new AvgBucketPipelineAggregation("avg_bucket", "histogram>sum");

		assertSearch(
			indexingTestHelper -> {
				Map<String, AggregationResult> aggregationResults = search(
					histogramAggregation, avgBucketPipelineAggregation);

				AvgBucketPipelineAggregationResult
					avgBucketPipelineAggregationResult =
						(AvgBucketPipelineAggregationResult)
							aggregationResults.get("avg_bucket");

				Assert.assertNotNull(avgBucketPipelineAggregationResult);
