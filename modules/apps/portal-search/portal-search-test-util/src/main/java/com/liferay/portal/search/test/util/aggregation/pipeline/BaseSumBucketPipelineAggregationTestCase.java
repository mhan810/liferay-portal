...
public abstract class BaseSumBucketPipelineAggregationTestCase
	...
	public void testSumBucketPipeline() throws Exception {
		...
		SumBucketPipelineAggregation sumBucketPipelineAggregation =
			new SumBucketPipelineAggregation("sum_bucket", "histogram>sum");

		assertSearch(
			indexingTestHelper -> {
				Map<String, AggregationResult> aggregationResults = search(
					histogramAggregation, sumBucketPipelineAggregation);

				SumBucketPipelineAggregationResult
					sumBucketPipelineAggregationResult =
						(SumBucketPipelineAggregationResult)
							aggregationResults.get("sum_bucket");

				Assert.assertNotNull(sumBucketPipelineAggregationResult);
