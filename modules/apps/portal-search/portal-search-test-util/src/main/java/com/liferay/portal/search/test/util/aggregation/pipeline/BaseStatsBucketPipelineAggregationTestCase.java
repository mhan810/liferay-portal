...
public abstract class BaseStatsBucketPipelineAggregationTestCase
	...
	public void testStatsBucketPipeline() throws Exception {
		...
		StatsBucketPipelineAggregation statsBucketPipelineAggregation =
			new StatsBucketPipelineAggregation("stats_bucket", "histogram>sum");

		assertSearch(
			indexingTestHelper -> {
				Map<String, AggregationResult> aggregationResults = search(
					histogramAggregation, statsBucketPipelineAggregation);

				StatsBucketPipelineAggregationResult
					statsBucketPipelineAggregationResult =
						(StatsBucketPipelineAggregationResult)
							aggregationResults.get("stats_bucket");

				Assert.assertNotNull(statsBucketPipelineAggregationResult);
