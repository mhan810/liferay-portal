...
package com.liferay.portal.search.test.util.aggregation.metrics.percentiles;
...
public abstract class BasePercentilesAggregationTestCase
	...
	public void testPercentilesAggregation() throws Exception {
		...
		PercentilesAggregation percentilesAggregation =
			new PercentilesAggregation("percentiles", Field.PRIORITY);

		percentilesAggregation.setPercentilesMethod(PercentilesMethod.HDR);

		PercentilesAggregationResult percentilesAggregationResult = search(
		...
		double[] values = {
			10, 10, 30.0078125, 50.0234375, 80.0546875, 100.0546875, 100.0546875
		};

		for (int i = 0; i < percents.length; i++) {
			Assert.assertEquals(values[i], percentilesMap.get(percents[i]), 0);
		}
	}
