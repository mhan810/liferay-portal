...
package com.liferay.portal.search.test.util.aggregation.bucket.global;
...
public abstract class BaseGlobalAggregationTestCase
	...
	public void testGlobalAggregation() throws Exception {
		...
		GlobalAggregation globalAggregation = new GlobalAggregation("global");
		...
		GlobalAggregationResult globalAggregationResult = search(
			globalAggregation);

		Assert.assertEquals(7, globalAggregationResult.getDocCount(), 0);

		Map<String, AggregationResult> childrenAggregationResults =
			globalAggregationResult.getChildrenAggregationResults();

		Assert.assertEquals(
			"Children aggregations", 1, childrenAggregationResults.size());

		AvgAggregationResult avgAggregationResult =
			(AvgAggregationResult)childrenAggregationResults.get("avg");
