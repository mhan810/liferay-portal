...
public abstract class BasePercentileRanksAggregationTestCase
	...
	@Test
	public void testPercentileRanksAggregation1() throws Exception {
		...
		PercentileRanksAggregationResult percentileRanksAggregationResult =
			search(percentileRanksAggregation);
		...
		for (int i = 0; i < values.length; i++) {
			Assert.assertEquals(percents[i], percentilesMap.get(values[i]), 0);
		}
	}

	@Test
	public void testPercentileRanksAggregation2() throws Exception {
		addDocument(DocumentCreationHelpers.singleNumber(Field.PRIORITY, 10));
		addDocument(DocumentCreationHelpers.singleNumber(Field.PRIORITY, 10));
		addDocument(DocumentCreationHelpers.singleNumber(Field.PRIORITY, 10));
		addDocument(DocumentCreationHelpers.singleNumber(Field.PRIORITY, 10));
