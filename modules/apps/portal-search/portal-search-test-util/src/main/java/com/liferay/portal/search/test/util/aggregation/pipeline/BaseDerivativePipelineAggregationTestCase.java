...
public abstract class BaseDerivativePipelineAggregationTestCase
	...
	public void testDerivative() throws Exception {
		...
		DerivativePipelineAggregation derivativePipelineAggregation =
			new DerivativePipelineAggregation("derivative", "sum");
		...
		assertSearch(
			indexingTestHelper -> {
				HistogramAggregationResult histogramAggregationResult = search(
	...
	protected void assertBucket(
		...
		if (derivativeValue != null) {
			...
			Assert.assertEquals(
				...
				derivativePipelineAggregationResult.getValue(), 0);
		}
	}
