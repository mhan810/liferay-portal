...
public abstract class BaseMinBucketPipelineAggregationTestCase
	...
	public void testMinBucketPipeline() throws Exception {
		...
		MinBucketPipelineAggregation maxBucketPipelineAggregation =
			new MinBucketPipelineAggregation("min_bucket", "histogram>sum");

		assertSearch(
			indexingTestHelper -> {
				Map<String, AggregationResult> aggregationResults = search(
					histogramAggregation, maxBucketPipelineAggregation);

				MinBucketPipelineAggregationResult
					minBucketPipelineAggregationResult =
						(MinBucketPipelineAggregationResult)
							aggregationResults.get("min_bucket");

				Assert.assertNotNull(minBucketPipelineAggregationResult);
