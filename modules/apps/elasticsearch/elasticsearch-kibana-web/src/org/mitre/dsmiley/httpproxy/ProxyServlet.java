 ...
 * <p> There are alternatives to a servlet based proxy such as Apache mod_proxy
 * if that is available to you. However this servlet is easily customizable by
 ...
 * application. </p> <p> Inspiration:
 * http://httpd.apache.org/docs/2.0/mod/mod_proxy.html </p>
 *
...
public class ProxyServlet extends HttpServlet {
	...
	protected HttpClient createHttpClient(HttpParams hcParams) {
		...
		return new DefaultHttpClient(
			new ThreadSafeClientConnManager(), hcParams);
	}
	 ...
	 */
	protected void readConfigParam(
		HttpParams hcParams, String hcParamName, Class type) {
		...
		if (type == String.class) {
			...
			try {
				...
				val_obj = type.getMethod("valueOf", String.class).invoke(
					type, val_str);
			}
			catch (Exception e) {
				throw new RuntimeException(e);
	...
	public void destroy() {
		...
		if (proxyClient instanceof Closeable) {// TODO AutoCloseable in Java 1.6
			try {
				...
				log(
					"While destroying servlet, shutting down HttpClient: " + e,
					e);
			}
		}
		else {
	...
	protected void service(
		...
		try {
			...
			proxyResponse = proxyClient.execute(
				getTargetHost(servletRequest), proxyRequest);
	...
	protected boolean doResponseRedirectOrNotModifiedLogic(
		...
		&& statusCode < HttpServletResponse.SC_NOT_MODIFIED /* 304 */) {
			...
			if (locationHeader == null) {
				throw new ServletException(
					"Received status code: " + statusCode + " but no " +
						HttpHeaders.LOCATION +
						" header was found in the response");
			}
			// Modify the redirect to go to this proxy servlet rather that the
			// proxied host
			String locStr = rewriteUrlFromResponse(
	...
	protected void copyResponseHeaders(
		HttpResponse proxyResponse, HttpServletRequest servletRequest,
	...
	protected void copyProxyCookie(
		...
		for (HttpCookie cookie : cookies) {
			...
			Cookie servletCookie =
				new Cookie(proxyCookieName, cookie.getValue());
		...
		for (String cookie : cookies) {
			...
			if (cookieSplit.length == 2) {
				...
				if (cookieName.startsWith(getCookieNamePrefix())) {
					...
					escapedCookie.append(cookieName).append("=").append(
						cookieSplit[1]);
				}
			}
	...
	protected void copyResponseEntity(
		...
		if (entity != null) {
			...
			Header header = proxyResponse.getFirstHeader("Content-encoding");
	...
	protected String rewriteUrlFromRequest(HttpServletRequest servletRequest) {
		...
		queryString =
			rewriteQueryStringFromRequest(servletRequest, queryString);
	...
	protected String rewriteUrlFromResponse(
		...
		if (theUrl.startsWith(targetUri)) {
			...
			if (pathInfo != null) {
				...
				curUrl =
					curUrl.substring(0, curUrl.length() - pathInfo.length());// take
																				// pathInfo
																				// off
			}
	 ...
	 * <p>Unfortunately, an incoming URI sometimes has characters disallowed by
	 * the spec. HttpClient insists that the outgoing proxied request has a
	 * valid URI because it uses Java's {@link URI}. To be more forgiving, we
	 * must escape the problematic characters. See the URI class for the spec.
	 *
	 * @param in
