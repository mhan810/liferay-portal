...
public abstract class BaseCompanySettingsVerifyProcessTestCase
	...
	public void setUp() throws Exception {
		...
		ServiceReference<CompanyLocalService> companyLocalServiceReference =
			bundleContext.getServiceReference(CompanyLocalService.class);

		companyLocalService = bundleContext.getService(
			companyLocalServiceReference);

		ServiceReference<PrefsProps> prefsPropsServiceReference =
			bundleContext.getServiceReference(PrefsProps.class);

		prefsProps = bundleContext.getService(prefsPropsServiceReference);

		ServiceReference<SettingsFactory> configurationAdminServiceReference =
			bundleContext.getServiceReference(SettingsFactory.class);

		settingsFactory = bundleContext.getService(
	...
	protected VerifyProcess getVerifyProcess() {
		try {
			...
			return (VerifyProcess)bundleContext.getService(
				serviceReferences[0]);
		}
